...
bool D3DManager::Initialize(int screenWidth, int screenHeight, bool vsync, HWND hwnd, bool fullscreen, float screenDepth, float screenNear)
{
	...
	size_t stringLength;//type used to be unsigned long long
	...
	//get the num of modes tht fit the DXGI_FORMAT_R8G8B8A8_UNORM display format for the monitor (adapter output)
	result = adapterOutput->GetDisplayModeList(DXGI_FORMAT_R8G8B8A8_UNORM, DXGI_ENUM_MODES_INTERLACED, &numModes, 0);
	if (FAILED(result))
	{
		return false;
	}
	...
	//go through all the display modes and find the one that best matches the screen width and height
	//when a match is found store the numerator and denomenator of the refresh rate for that monitor
	...
	static_cast<int>(adapterDesc.DedicatedVideoMemory / 1024 / 1024);
	...
	//create the swapchain, direct32 device & direct32 device context
	result = D3D11CreateDeviceAndSwapChain(0, D3D_DRIVER_TYPE_HARDWARE, 0, 0, &featureLevel, 1, D3D11_SDK_VERSION, &swapChainDesc,
		&m_swapChain, &m_device, 0, &m_deviceContext);
	if (FAILED(result))
	{
		return false;
	}
	...
	//initialize the descrption of the depth  buffer
	...
	//create a depth stencil state using its desc
	...
	//create an orthographic projectoin matrix for 2D rendering
...
{
	//set to windowed mode before shutting down
	if (m_swapChain)
	{
...
void D3DManager::GetViewCardInfo(char * cardName, int & memory)
{
